...
const GitHubImg = ({ src, alt }: { src: string; alt: string }) => {
  const [state, setState] = useState<ImgState>('loading');

  return (
    <div className="relative w-full flex justify-center items-center min-h-[195px]">
      {state === 'loading' && (
      ...
      <img
        src={src}
        alt={alt}
        className={`rounded-lg w-full max-w-[495px] object-contain transition-opacity duration-300 ${state === 'loaded' ? 'opacity-100' : 'opacity-0'}`}
...
const GitHubWidget = () => {
  ...
  const streakSrc = `https://github-readme-streak-stats.herokuapp.com/?user=${username}&theme=${isDark ? 'radical' : 'default'}&hide_border=true&background=${isDark ? '1a1b27' : 'ffffff'}&ring=10b981&fire=10b981&currStreakLabel=10b981`;
  const langsSrc = `https://github-readme-stats.vercel.app/api/top-langs/?username=${username}&layout=compact&theme=${isDark ? 'radical' : 'default'}&hide_border=true&bg_color=${isDark ? '1a1b27' : 'ffffff'}&text_color=${isDark ? 'ffffff' : '374151'}&title_color=10b981`;
    ...
    <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-xl border-2 border-gray-200 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
          <GitHubImg src={streakSrc} alt="GitHub Streak" />
        </div>
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
          <GitHubImg src={langsSrc} alt="Most Used Languages" />
        </div>
      </div>
